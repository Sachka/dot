...
function __promptline_left_prompt {
  ...
  [ $is_prompt_empty -eq 1 ] && slice_prefix="$slice_empty_prefix"
  # section "a" slices
  __promptline_wrapper "$USER | $HNAME |" "$slice_prefix" "$slice_suffix" && { slice_prefix="$slice_joiner"; is_prompt_empty=0; }

  # section "b" header
  slice_prefix="${b_bg}${sep}${b_fg}${b_bg}${space}" slice_suffix="$space${b_sep_fg}" slice_joiner="${b_fg}${b_bg}${alt_sep}${space}" slice_empty_prefix="${b_fg}${b_bg}${space}"
  [ $is_prompt_empty -eq 1 ] && slice_prefix="$slice_empty_prefix"
  # section "b" slices
  #__promptline_wrapper "$HNAME" "$slice_prefix" "$slice_suffix" && { slice_prefix="$slice_joiner"; is_prompt_empty=0; }
...
function __promptline {
  ...
  local reset_bg="${wrap}0${end_wrap}"
  local a_fg="${wrap}0${end_wrap}"
  local a_bg="${wrap}0${end_wrap}"
  local a_sep_fg="${wrap}0${end_wrap}"
  local b_fg="${wrap}0${end_wrap}"
  local b_bg="${wrap}0${end_wrap}"
  local b_sep_fg="${wrap}0${end_wrap}"
  local c_fg="${wrap}0${end_wrap}"
  local c_bg="${wrap}0${end_wrap}"
  local c_sep_fg="${wrap}0${end_wrap}"
  local warn_fg="${wrap}0${end_wrap}"
  local warn_bg="${wrap}0${end_wrap}"
  local warn_sep_fg="${wrap}0${end_wrap}"
  local y_fg="${wrap}0${end_wrap}"
  local y_bg="${wrap}0${end_wrap}"
  local y_sep_fg="${wrap}0${end_wrap}"
  if [[ -n ${ZSH_VERSION-} ]]; then
